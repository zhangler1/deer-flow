...
<cite>
**本文档引用的文件**
- [src/config/tools.py](file://src/config/tools.py) - *更新了默认搜索引擎*
- [src/config/custom_search.py](file://src/config/custom_search.py) - *更新了默认仓库配置*
- [src/tools/search.py](file://src/tools/search.py) - *搜索工具实现*
- [src/tools/custom_search.py](file://src/tools/custom_search.py) - *自定义搜索工具实现*
- [conf.yaml](file://conf.yaml) - *主配置文件，已更新默认值*
...
## 更新摘要
**已更新内容**
- 更新了默认搜索引擎为 `custom_search`
- 更新了自定义搜索的默认仓库为 `aggregation_search`
- 修正了核心工具配置和搜索工具配置中的过时信息
- 更新了自定义搜索引擎配置部分的默认仓库信息

**文档来源更新**
- 新增了对 `conf.yaml` 文件的引用，因其包含关键的默认配置
- 更新了 `src/config/tools.py` 和 `src/config/custom_search.py` 的引用状态，标记为已更新
...
默认搜索引擎通过环境变量 `SEARCH_API` 进行配置，**其默认值已更新为 `custom_search`**。
...
| 变量名 | 类型 | 默认值 | 描述 |
|--------|------|--------|------|
| `SEARCH_API` | String | "custom_search" | 搜索引擎选择，**已更新默认值** |
