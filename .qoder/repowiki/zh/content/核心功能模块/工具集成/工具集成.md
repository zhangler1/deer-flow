...
- [crawl.py](file://src/tools/crawl.py) - *在最近提交中已移除*
- [retriever.py](file://src/tools/retriever.py)
</cite>

## 更新摘要
**已做更改**
- 更新了搜索工具详解章节，增加了Arxiv和Wikipedia搜索参数的详细说明
- 更新了Brave搜索工具的安全配置说明，增加了SecretStr的使用说明
- 更新了配置管理章节中的搜索引擎配置示例
- 移除了关于`crawl_tool`的过时内容，反映代码库中的实际变更
- 所有文件引用均已更新为中文标题
...
- [crawl.py](file://src/tools/crawl.py#L1-L30) - *在最近提交中已移除*
...
Arxiv搜索工具专门用于学术论文检索，支持以下参数配置：

- **top_k_results**：返回的最高结果数
- **load_max_docs**：加载的最大文档数
- **load_all_available_meta**：是否加载所有可用的元数据
- **arxiv_search**：自定义Arxiv搜索参数
- **arxiv_exceptions**：异常处理配置
...
Wikipedia搜索工具支持多语言维基百科内容检索，支持以下参数配置：

- **lang**：维基百科语言版本（如"zh"表示中文，"en"表示英文）
- **top_k_results**：返回的最高结果数
- **load_all_available_meta**：是否加载所有可用的元数据
- **doc_content_chars_max**：文档内容的最大字符数
- **wiki_client**：自定义维基客户端
...
**章节来源**
- [search.py](file://src/tools/search.py#L100-L114)
...
Brave搜索工具现在使用pydantic的SecretStr来保护API密钥，确保敏感信息不会被意外泄露：
...
这种实现方式确保了API密钥在日志记录和调试输出中被安全地隐藏，提高了系统的安全性。

**章节来源**
- [search.py](file://src/tools/search.py#L75-L80)
...
2. **检查工具状态**
   ...
   print([python_repl_tool, get_web_search_tool, get_retriever_tool, VolcengineTTS])
   ```
