...
- [src/graph/builder.py](file://src/graph/builder.py) - *在最近的提交中更新*
</cite>

## 更新摘要
**已做更改**
- 更新了“Web界面配置映射”部分，以反映 `auto_accepted_plan` 配置项的默认值变更
- 更新了“全局参数配置”部分，以反映 `auto_accepted_plan` 配置项的变更
- 更新了“配置优先级”和“故障排除”部分，以包含 `auto_accepted_plan` 的相关信息
...
| `auto_accepted_plan` | bool | False | 是否自动接受生成的计划，无需人工审核 |

**章节来源**
- [src/config/configuration.py](file://src/config/configuration.py#L25-L70)
- [src/graph/types.py](file://src/graph/types.py#L15-L24) - *在最近的提交中更新*
...
const generalFormSchema = z.object({
  autoAcceptedPlan: z.boolean().default(false),
...
**问题症状**：`auto_accepted_plan` 配置未按预期工作

**解决方案**：
- 确认 `auto_accepted_plan` 在配置文件或环境变量中正确设置
- 检查环境变量 `AUTO_ACCEPTED_PLAN` 的值是否为 `true`、`1`、`yes` 或 `y`
- 重启服务以确保配置生效
- 查看日志确认 `auto_accepted_plan` 的实际值
...
- [src/graph/builder.py](file://src/graph/builder.py#L25-L115) - *在最近的提交中更新*