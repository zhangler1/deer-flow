...
选择模板后，`report_style` 传对应 `template_id` 到后端，加载对应提示词执行研究。
...
- `report-style-dialog.tsx`：替换为模板选择器组件，从 `/api/templates` 获取列表
- 新增页面：「我的模板」、「管理后台」
- 新增路由 + 权限守卫（管理后台仅 admin 可见）
