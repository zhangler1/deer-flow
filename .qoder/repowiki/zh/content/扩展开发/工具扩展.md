...
```python
def inspect_tool_state(tool_instance):
    """检查工具的状态和配置"""
    print(f"工具名称: {tool_instance.name}")
    print(f"工具描述: {tool_instance.description}")
    print(f"参数模式: {tool_instance.args_schema}")
    print(f"工具类型: {type(tool_instance)}")
...
```python
def integration_test_tool(tool_name: str, test_cases: List[Dict]):
    """集成测试工具"""
