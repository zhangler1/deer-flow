...
条件路由是 deer-flow 的核心特性之一，它允许根据当前状态动态决定流程走向：

```mermaid
flowchart TD
Start([研究团队节点]) --> CheckPlan{"检查当前计划"}
CheckPlan --> |无计划或无步骤| Planner[跳转到规划器]
CheckPlan --> |所有步骤已完成| Planner
CheckPlan --> |存在未完成步骤| FindStep[查找第一个未完成步骤]
FindStep --> CheckType{"检查步骤类型"}
CheckType --> |研究类型| Researcher[跳转到研究员]
CheckType --> |处理类型| Coder[跳转到编码器]
CheckType --> |其他类型| Planner
Researcher --> UpdateState[更新状态]
Coder --> UpdateState
Planner --> UpdateState
UpdateState --> End([结束])
```
...
```python
def monitor_graph_performance(graph: StateGraph):
    """监控图执行性能。"""
    import time
    import psutil
