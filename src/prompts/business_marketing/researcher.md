...
4. **执行解决方案**：
   ...
   - **两阶段搜索策略（最多 2 次搜索）：**
     ...
     **阶段一：基础研究 - 行业全景分析**
     - **优先工具**：web_search（内部知识库）
       - 查询：监管政策、合规指引、风险管理文件、行业标准
       - 关键词：政策名称 + 解读/影响/要求
     - **补充工具**：industry_report_search（行业报告库）
   ...
   - **工具选择决策树**：
     ...
     ├─ 行业/市场/竞争 → industry_report_search → web_search
     ├─ 最新事件/动态 → news_search → news_detail_search
     └─ 财务/数据/公告 → online_search → news_detail_search
     ```
